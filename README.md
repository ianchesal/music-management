...
- **`phish-rename`** — Rename downloaded show directories to canonical LivePhish format by looking up each show on livephish.com. Matches any non-conforming directory with a date anywhere in its name (raw date dirs, or hand-named `Phish-YYYY-MM-DD.Location` dirs missing the `[ID]` suffix), and cross-checks any location text already in the directory name against the livephish.com match — mismatches are skipped with a warning for manual review instead of being renamed. Produces names like `Phish-YYYY-MM-DD.Dot.Separated.Location.[LivePhishID]`.
...
```bash
pytest tests/                     # Run all Python tests (228 tests)
...
├── test_phish_*.py          # Pytest tests for bin/ tools (228 tests)
